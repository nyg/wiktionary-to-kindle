...
- **`edu.self.w2k.gui`** — JavaFX front-end. `Launcher` (plain `main`, **must not** extend `Application` — a classpath launch otherwise fails with "JavaFX runtime components are missing"), `App` (FXML load, appender install), `MainViewModel` (state and derivations; `javafx.base` only, so unit-testable without a toolkit), `MainController` (bindings only), `PipelineService`/`PipelineTask` (threading + cancellation), `UiLogAppender`, `ProgressSnapshot`, `PreferencesDialog`, `ByteSizes`, `LanguageConverter` (resolves typed text against the languages a picker currently offers, `null` for anything else), `WordLanguageConverter` (same, appending the sense count), `ComboBoxFilter` (type-to-filter on the pickers, plus the drop-down's scroll fallback), `LogClipboard` (copying console lines), `LogFile` (the daily-rolling log file), `SystemTheme` (AtlantaFX Cupertino on macOS, following the system colour scheme). FXML and CSS in `src/main/resources/edu/self/w2k/gui/`
...
- **`edu.self.w2k.write`** — `DictionaryWriter` interface, `DictionaryTitles` utility, `IntermediateFiles` (the per-dictionary working directory and its cleanup)
...
| `dictionaries/` | Final `.mobi` dictionary files |
| `dictionaries/intermediate/{base name}/` | One working directory per dictionary, holding the `.opf`, `-N.html`, `-toc.ncx` and `-cover.jpg` that build it |
...
`KindlingDictionaryConverter` composes `OpfDictionaryWriter` with a `kindling-cli` binary. It calls `KindlingCliResolver.resolve()` to obtain the binary, then runs `kindling-cli build <opf> -o <mobi>`. Output: `dictionaries/w2k-dictionary-{src}-{trg}.mobi`.

**The side-artefacts go in a sub-folder, the `.mobi` does not.** `IntermediateFiles.dirFor` puts them under `dictionaries/intermediate/w2k-dictionary-{src}-{trg}/` — one directory per dictionary, which keeps two editions' runs apart and makes the optional cleanup a single recursive delete rather than a name-prefix sweep over a shared folder. The converter passes that directory to `OpfDictionaryWriter` and keeps `outputDir` for the `.mobi` alone; kindling-cli resolves the OPF's hrefs relative to the OPF and every file it names moves together, so it never sees the difference. `Preferences.deleteIntermediateFiles` (default `false`) drops the working directory once the `.mobi` exists, and `IntermediateFiles.delete` removes the shared `intermediate` parent too when that leaves it empty. The delete runs **only after a zero exit code** and never fails the run: on a kindling-cli failure the files are the only way to diagnose it, and after a success a cleanup error is an annoyance, not a broken result.
...
- **Focus opens the drop-down.** Reaching a picker offers its list straight away, with the filter cleared, rather than making the user type or hunt for the arrow. The arrow is the exception and needs the `arrowPressed` guard in `ComboBoxFilter`: it toggles the popup on mouse *release*, so opening on the focus the same press delivers would leave the toggle to close it again and a click on the arrow would do nothing.
- **A drop-down scroll must not depend on what the pointer is over.** `VirtualFlow` registers its scroll handler on itself, so only an event whose target lies inside it moves the list; one landing on the popup's own padding — AtlantaFX gives it 5px — or on any other gap the theme introduces bubbles straight past and does nothing, and the list appears to stop scrolling part-way down (issue #85). `ComboBoxFilter.keepScrollingWhereverThePointerIs` handles `ScrollEvent.SCROLL` on the popup's `ListView` and applies anything still unconsumed to the flow. The `isConsumed` guard is what stops it double-scrolling: `VirtualFlow` consumes exactly when `scrollPixels` moved something. It hangs off `skinProperty` because the skin, and therefore the popup content, does not exist until the control reaches a shown scene.
- **The word language picker is disabled until an edition is chosen.** The languages it offers are that edition's own, so before one is picked it can only show the bundled fallback — and picking from it invited a language the edition does not serve. `MainViewModel.wordLanguageSelectable` is the binding; `MainController.onEditionChanged` also clears the word language when the edition goes away, since a disabled picker still showing a value from a list that no longer applies cannot be cleared by the user.
...
- **The word language list is scoped to the selected edition.** Offering all ~184 ISO 639-1 codes against, say, dewiktionary's 60 languages meant most choices produced an empty dictionary after a multi-gigabyte download. `LanguageCodeResolver` maps kaikki's localised names to the dump's `lang_code` — the bundled `kaikki-language-codes.properties` first, then `Locale.getDisplayLanguage` in the edition's own locale. **Unmatched names are dropped, never guessed**, because `JsonlDictionaryParser` filters on an exact `lang_code` match and a wrong code silently yields an empty dictionary. Regenerate the alias table by reading each per-language dump's first JSONL line, as its header describes — parse that line as JSON and take the *top-level* `lang_code`, since a nested one under `translations` will otherwise match first and be wrong. An HTTP `Range: bytes=0-400000` on that dump is enough and avoids the multi-gigabyte download; when the dump's filename does not match its directory, read the language's `index.html` and follow the `.jsonl` link it carries.
- **`LanguageCodeResolver.normalise` keeps letters in every script, not just `a-z`.** It strips accents through NFD and drops everything that is neither a letter nor a digit, but `[^a-z0-9]` deleted whole names: every Chinese, Cyrillic, Japanese, Korean and Thai name normalised to the empty string, `codeFor` returned empty for all of them, and `MainController` fell back to the bundled ISO 639-1 list — so five editions offered ~184 alphabetical languages with no sense counts and no relation to what kaikki serves. It also dropped Latin letters that NFD does not reduce to ASCII, losing `język włoski`, `Osmanlı Türkçesi` and `Tiếng Quảng Đông`. All 2081 languages across the 21 editions now resolve; the table carries 1103 entries, keyed in the edition's own script.
- **The bundled properties files are UTF-8, so they are read through a `Reader`.** `Properties.load(InputStream)` decodes ISO-8859-1 whatever the file actually is, and a handful of kaikki codes are not ASCII — `solrésol`, `bété`, `grec cargésien`, `tsolyáni` — so the alias table was loaded as mojibake and the picker rendered `Solrésol (solrÃ©sol)` back at the user (issue #85). `LanguageCodeResolver` and `LanguageCatalog` both pass an explicit UTF-8 `InputStreamReader` to `load`. `Preferences` deliberately does not: it both writes and reads its file with the `Properties` escaping rules, so it round-trips any character already, and switching one side alone would break existing files.
...
- **Logging** uses SLF4J 2.x with Logback Classic. `@Slf4j` (Lombok) is used on all classes. `logback.xml` configures the CLI console only; the GUI adds `UiLogAppender` and a file appender **programmatically**, so CLI output is untouched. `LogFile.install` is that file appender: a `RollingFileAppender` rolling daily, active file `app.log`, each day's predecessor compressed beside it as `app-{YYYY-MM-DD}.log.gz`, `maxHistory` 14 days and a 200 MB `totalSizeCap`. It appends rather than truncating at startup, which is what the old `FileAppender` did — truncation was the only thing bounding the file, and it cost the user every earlier session in the same sitting, so a crash reproduced on the second run erased the evidence of the first. `cleanHistoryOnStart` is set because a rolling policy prunes when it rolls and a desktop app opened and closed within a day may go weeks without rolling. Extracted from `App` so it is testable: `App` is in `sonar.coverage.exclusions`, and a bare `LoggerContext` in a test needs `setMDCAdapter(new LogbackMDCAdapter())` or every append fails on a null adapter. `UiLogAppender` buffers into a bounded queue the UI drains in batches — never one `Platform.runLater` per event, which would freeze the window. The drain `Timeline` parks itself after ~2 s idle and `UiLogAppender.setWakeListener` restarts it, so an idle window is not held awake by a permanently running animation. `LogClipboard` makes the pane's lines copyable — multiple selection, `Shortcut+C`, and a Copy / Copy all context menu — because a `ListView` is not a text control and carries no copy behaviour of its own, which left a failure the user wanted to paste into a bug report retypeable only by hand.
...
- **The script takes one or more types** (`scripts/package.sh app-image exe`, `scripts/package.sh deb rpm`), defaulting to `detect_default_type` for the host. Collecting the JavaFX modules, linking the runtime image and running the probe are type-independent and happen once; only `jpackage` runs per type. `rm -rf "$OUTPUT_DIR"` therefore lives in `main`, **before** the loop — it used to open `build_package`, and left there it deletes the previous type's output, so a two-type run would publish one file.
- **`platform_arguments` takes the type**, because the installer flags are type-specific: `--win-menu --win-dir-chooser --win-per-user-install --win-shortcut` apply to `msi` and `exe` but not to `app-image`, and `--linux-rpm-license-type` only to `rpm`. The Windows `exe` is deliberately per-user — no admin rights and no UAC prompt, matching what Scoop already gives — and unsigned, so SmartScreen prompts on first run; both facts are documented rather than solved.
- **`--linux-package-name` is pinned to the slug** rather than derived. jpackage derives it from `--name`, which is `Wiktionary to Kindle`, and deb and rpm names admit neither spaces nor capitals; the pinned value is also the name the uninstall instructions document.
- **jpackage cannot produce a portable single-file `.exe`.** The `exe` type is a WiX-built installer; the app-image is always a directory of launcher, `runtime/` and `app/`, which is what the Scoop ZIP carries. The two Windows assets are alternatives, not duplicates. jpackage 24 and later accept WiX v3, v4 and v5 and pick the newest installed; the runner image ships v3.14 but does not put it on the `PATH`, so `release.yml` adds it by globbing `C:\Program Files (x86)\WiX Toolset v*\bin`.

## CI & Release

- `java-ci.yaml` — `build` runs `mvn verify` under `xvfb-run` on Ubuntu; `package` runs jlink+jpackage on macOS (`dmg`) and Windows (`app-image`), so a packaging regression in those two is caught in a PR rather than at release time. It deliberately does **not** cover `exe`, `deb` or `rpm`: those three are release-only, so a WiX, `fakeroot` or `rpmbuild` breakage surfaces when a release is cut. Do not assume PR coverage the matrix does not have.
- `release.yml` — `workflow_dispatch` with a bump choice. Bumps via `maven-release-plugin` (hence the `-SNAPSHOT` version and `<scm>` block), builds the DMG, the Windows installer and Scoop ZIP, the deb, the rpm and the portable JAR, publishes, then dispatches to `nyg/homebrew-tap` and pushes a manifest to `nyg/scoop-bucket`. `RELEASE_TOKEN` needs write access to all three repos. The `linux` matrix leg produces the deb, the rpm **and** the cross-platform JAR — the JAR is not Linux-specific, it just needs some runner; and that leg `apt-get install`s `fakeroot` and `rpm` rather than trusting the image inventory.
- **Release asset names** are `wiktionary-to-kindle-<version>-<os>-<arch>.<ext>` — `…-macos-arm64.dmg`, `…-windows-x64.exe`, `…-windows-x64-scoop.zip`, `…-linux-x64.deb`, `…-linux-x64.rpm`, and the plain `wiktionary-to-kindle-<version>.jar` (cross-platform, so no os/arch suffix). jpackage's own names differ on every platform (`wiktionary-to-kindle_X.Y.Z_amd64.deb`, `Wiktionary to Kindle-X.Y.Z.exe`), so each build step renames by extension glob rather than by full name. Every reference interpolates `VERSION` from a step-level `env`: the rename steps, the 7z output, `gh release create`, both `sha256sum` inputs and the Scoop manifest url. Only the DMG and the Scoop ZIP are hashed — nothing downstream consumes the exe, deb or rpm. Renaming an asset means updating the Homebrew cask in `nyg/homebrew-tap` too, and only **after** a release has published under the new name — the Scoop bucket regenerates itself.
- **The release notes are generated, then appended to.** `git-cliff` writes the changelog and the `Append install guidance` step adds the Installing section as a heredoc. Filenames in it are written with a literal `VERSION` and rewritten by one `sed` on `wiktionary-to-kindle-VERSION`, which covers every asset name; a new filename pattern that does not start that way needs its own substitution.
